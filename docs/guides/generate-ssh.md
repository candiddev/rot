...
You'll need to generate a private key to create a SSH keypair and an SSH CA to sign the certificates.  The easiest way to do this is using {{% cli value-private %}} (encrypting the keys into Rot) or {{% cli key-new %}} (printing the keys to stdout).
